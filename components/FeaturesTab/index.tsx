...
import data from "@/app/(site)/data";
import { motion } from "framer-motion";
import SectionHeader from "../Common/SectionHeader";

const FeaturesTab = () => {
  const [currentTab, setCurrentTab] = useState(data[4].services[0].title);
    ...
    <>
      ...
      <section className="relative pb-20 pt-18.5 lg:pb-22.5">
        ...
        <div className="relative mx-auto max-w-c-1390 px-4 md:px-8 2xl:px-0">
          ...
          >
            {data[4].services.map((service, index) => (
              <div
                key={index}
                onClick={() => setCurrentTab(service.title)}
          ...
          >
            {data[4].services.map((service, key) => (
              <div
                className={currentTab === service.title ? "block" : "hidden"}
                key={key}
              >
                <FeaturesTabItem featureTab={service} />
              </div>
            ))}
          </motion.div>
          {/* <!-- Tab Content End --> */}
